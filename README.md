...
本仓库新增了一个简单的 FastAPI 服务示例，提供 `/api/v2/devices/active` 接口用于获取活动设备列表。
...
3. 调用接口示例：
   ```bash
   curl -X POST "http://127.0.0.1:8000/api/v2/devices/active" -H "Content-Type: application/json" -d '{"category": "router"}'
   ```
   若未提供 `category` 字段，将返回所有 `is_active` 为 `true` 的设备。
